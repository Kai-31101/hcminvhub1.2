export const administrativeLocationOptions = [
  'Phường Sài Gòn',
  'Phường Tân Định',
  'Phường Bến Thành',
  'Phường Cầu Ông Lãnh',
...
export function getAdministrativeLocationLabel(value: string, language: 'en' | 'vi') {
  if (language === 'vi') return value;

  if (value.startsWith('Phường ')) {
    return `${stripVietnameseDiacritics(value.slice('Phường '.length))} Ward`;
  }
  if (value.startsWith('Xã ')) {
    return `${stripVietnameseDiacritics(value.slice('Xã '.length))} Commune`;
  }
  if (value.startsWith('Đặc khu ')) {
    return `${stripVietnameseDiacritics(value.slice('Đặc khu '.length))} Special Zone`;
  }

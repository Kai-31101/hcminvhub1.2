...
function buildDashboardStats(
  projects: Array<{ id: string; sector: string; budget: number }>,
  t: (value: string) => string,
) {
  const totalFollowers = projects.reduce((sum, project) => sum + getProjectFollowerCount(project), 0);
  ...
  return [
    ...
    { label: t('Follower'), value: formatFollowerCount(totalFollowers), tone: 'text-[#9d4300]' },
    { label: t('Investment Value'), value: formatPortfolioValue(projects.reduce((sum, project) => sum + project.budget, 0)), tone: 'text-emerald-700' },
  ];
}
...
export default function InvestorExecutionListPage() {
  ...
  const followedProjectsForDisplay = useMemo(() => {
    if (!activeFilter) return followedProjects;

    if (activeFilter.type === 'sector') {
      return followedProjects.filter((project) => project.sector === activeFilter.value);
    }

    if (activeFilter.type === 'project_status') {
      return followedProjects.filter((project) => getProjectStageLabel(project.status, project.stage) === activeFilter.value);
    }

    return followedProjects.filter((project) => (project.location || project.province) === activeFilter.value);
  }, [activeFilter, followedProjects, projectJobs]);

  const followedKeyStats = useMemo(() => buildDashboardStats(followedProjectsForDisplay, t), [followedProjectsForDisplay, t]);

  const groupedFollowedBySector = useMemo(
    () => buildCountGroups(followedProjectsForDisplay.map((project) => project.sector)),
    [followedProjectsForDisplay],
  );

  const groupedFollowedByProjectStatus = useMemo(
    () => buildCountGroups(followedProjectsForDisplay.map((project) => getProjectStageLabel(project.status, project.stage))),
    [followedProjectsForDisplay],
  );

  const groupedFollowedByLocation = useMemo(
    () => buildCountGroups(followedProjectsForDisplay.map((project) => project.location || project.province)),
    [followedProjectsForDisplay],
  );

  const locationOptions = useMemo(
    () =>
      Array.from(new Set(followedProjectsForDisplay.map((project) => project.location || project.province)))
        .filter(Boolean)
  ...
  const projectStatusOptions = useMemo(
    () =>
      Array.from(new Set(followedProjectsForDisplay.map((project) => getProjectStageLabel(project.status, project.stage))))
        .filter(Boolean)
        .sort((left, right) => left.localeCompare(right)),
    [followedProjectsForDisplay],
  );
  ...
  const projectAssignmentMap = useMemo(
    () =>
      Object.fromEntries(
        followedProjectsForDisplay.map((project) => {
    ...
    [agencies, followedProjectsForDisplay, projectJobs, users],
  );
  ...
  const projectAuditMap = useMemo(
    ...
    [followedProjectsForDisplay, users],
  );

  const projectJobAlertMap = useMemo(
    () =>
      Object.fromEntries(
        followedProjectsForDisplay.map((project) => [project.id, getProjectJobAlertSummary(project.id)]),
      ),
    [followedProjectsForDisplay, projectJobs],
  );

  const listFilteredProjects = useMemo(() => {
    const normalizedNameFilter = projectNameFilter.trim().toLowerCase();

    return followedProjectsForDisplay.filter((project) => {
  ...
  }, [followedProjectsForDisplay, locationFilter, projectNameFilter, projectStatusFilter]);
      ...
      <div className="page-shell">
        <div className="section-panel flex flex-col items-center gap-4 p-12 text-center">
          ...
          <p className="max-w-xl text-sm text-slate-500">
            {t('Projects will appear here so you can monitor sector mix, execution status, and project jobs in one investor workspace.')}
          </p>
  ...
  return (
    <div className="page-shell space-y-6">
      ...
      <section className="section-panel p-6">
        <div className="mb-4 flex items-center justify-between gap-3">
          <div>
            <h2 className="section-heading mb-0">{t('Watchlist Dashboard')}</h2>
            <p className="mt-1 text-sm text-slate-500">
              {t('Portfolio KPIs and grouped dashboards for the followed projects in this investor watchlist mockup.')}
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <StatusPill tone="info">{followedProjectsForDisplay.length}</StatusPill>
            {activeFilter ? (
              <>
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        ...
        <DashboardDonutChart
          ...
          rows={groupedFollowedByProjectStatus}
        ...
        <DashboardDonutChart
          ...
          rows={groupedFollowedByLocation}
          filterType="location"
          activeFilter={activeFilter}
          onSelect={handleFilterSelect}
          itemLabel={t('projects')}
      ...
      <section className="section-panel p-6">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="section-heading mb-0">{t('Watchlist Projects')}</h2>
            <p className="mt-1 text-sm text-slate-500">
              {activeFilter || projectNameFilter.trim() || locationFilter !== 'all' || projectStatusFilter !== 'all'
                ...
                : t('Project cards mirror the ITPC list behavior, and this mockup list is capped at 10 followed projects.')}
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="space-y-3">
          {visibleProjects.length > 0 ? (
            ...
            <div className="rounded-lg border border-dashed border-border px-4 py-10 text-center text-sm text-slate-500">
              {t('No followed projects match the current filter.')}
            </div>
          )}
      ...
      <section className="section-panel p-6">
        <div className="flex items-start gap-4">
          ...
          <div>
            <div className="text-sm font-semibold text-slate-900">{t('Watchlist scope')}</div>
            <div className="mt-1 text-sm leading-6 text-slate-600">
              {t('This mockup intentionally focuses on the 10 followed projects so investors can monitor their broader interest portfolio separately from the joined-project execution workspace.')}
            </div>
          </div>
        </div>
      </section>
